...
{%- block content %}
      <h1 class="small-title">{% block index_title %}« Todos los articulos »{% endblock index_title %}</h1>
      ...
      <article class="post">
        <header class="post-header">
          <h1>
            <a rel="bookmark"
               ...
               title="Enlace permanente a «{{ article.title }}»">
               {{ article.title }}
            </a>
          </h1>
          <div class="meta">
        ...
        <footer class="post-footer">
				<p><a  href="{{ SITEURL }}/{{ article.url }}">Leer más...</a></p>
        </footer>
      </article>
        {% set article = False %}
        {% endfor %}
