...
<html lang="{{ article.lang if article and article.lang else DEFAULT_LANG }}">
  ...
  <body>
	...
	<p class="site-desc">{% if SITESUBTITLE %}{{ SITESUBTITLE }}</p>
	<nav>
	      	<ul>
			 {% for title, link in MENUITEMS %}<li><a href="{{ link }}">{{ title }}</a></li>{% endfor %}
			 {% for cat, null in categories %}<li {% if cat == category %}class="active"{% endif %}><a href="{{ SITEURL }}/category/{{ cat }}.html">{{ cat|capitalize }}</a></li>{% endfor %}
			 <li><a href="{{ SITEURL }}/archives.html">Archivo</a></li>
			 <li><a href="{{ SITEURL }}/tags.html">Etiquetas</a></li>
			 {% if DISPLAY_PAGES_ON_MENU %}
			 {% for page in PAGES %}<li><a href="{{ SITEURL }}/{{ page.url }}">{{ page.title|capitalize }}</a></li>{% endfor %}
			 {% endif %}
